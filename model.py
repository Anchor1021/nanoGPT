...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.is_causal = config.is_causal # flag for attention
...
#Sliding Window Attenttion Implementation:
...
class MLP2(nn.Module): # q4 fc3(ReLU(fc1(x)) .* fc2(x)))
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = CausalSelfAttention(config) if config.is_causal else WindowAttention(config)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config) if config.mlp_type == 'org' else MLP2(config)
